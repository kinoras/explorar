...
const LeafletTileLayer = dynamic(async () => (await import('react-leaflet')).TileLayer, {
    ssr: false
})
...
const LeafletCircleMarker = dynamic(async () => (await import('react-leaflet')).CircleMarker, {
    ssr: false
})
const LeafletPolyline = dynamic(async () => (await import('react-leaflet')).Polyline, {
    ssr: false
})
const LeafletPolygon = dynamic(async () => (await import('react-leaflet')).Polygon, { ssr: false })
const LeafletRectangle = dynamic(async () => (await import('react-leaflet')).Rectangle, {
    ssr: false
})
const LeafletLayerGroup = dynamic(async () => (await import('react-leaflet')).LayerGroup, {
    ssr: false
})
const LeafletFeatureGroup = dynamic(async () => (await import('react-leaflet')).FeatureGroup, {
    ssr: false
})
...
const Map = ({
    ...
    return (
        <LeafletMapContainer
            ...
            zoomControl={false}
            className={cn('z-10 size-full min-h-96 flex-1 rounded-md', className)}
            {...props}
        />
    )
}
...
const MapLayersControl = ({
    ...
    const handleLayerGroupToggle = (name: string, checked: boolean) => {
        setActiveLayerGroups(
            checked
        ...
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <Button
                    ...
                    title="Select layers"
                    className={cn('border-border absolute top-1 right-1 z-1000 border', className)}
                    {...props}
                >
...
const MapPolyline = ({ className, ...props }: PolylineProps & { ref?: Ref<Polyline> }) => {
    return (
        <LeafletPolyline
            className={cn('fill-foreground stroke-foreground stroke-2', className)}
            {...props}
...
const MapPolygon = ({ className, ...props }: PolygonProps & { ref?: Ref<Polygon> }) => {
    return (
        <LeafletPolygon
            className={cn('fill-foreground stroke-foreground stroke-2', className)}
            {...props}
...
const MapRectangle = ({ className, ...props }: RectangleProps & { ref?: Ref<Rectangle> }) => {
    return (
        <LeafletRectangle
            className={cn('fill-foreground stroke-foreground stroke-2', className)}
            {...props}
...
const MapPopup = ({ className, ...props }: Omit<PopupProps, 'content'> & { ref?: Ref<Popup> }) => {
    return (
        <LeafletPopup
            className={cn(
                'bg-popover text-popover-foreground animate-in fade-out-0 fade-in-0 zoom-out-95 zoom-in-95 slide-in-from-bottom-2 border-border z-50 w-72 rounded-md border p-4 font-sans shadow-md outline-hidden',
                className
            )}
            {...props}
        />
...
const MapZoomControl = ({ className, ...props }: ComponentProps<'div'>) => {
    const map = useMap()
    ...
    return (
        <>
            <Button
                ...
                className={cn('border-border absolute right-1 bottom-1 z-1000 border', className)}
                {...props}
            >
                {isLocating ? (
...
const MapDrawShapeButton = <T extends Draw.Feature>({
    drawMode,
    createDrawTool,
    className,
    ...props
...
const MapDrawMarker = ({ ...props }: DrawOptions.MarkerOptions) => {
    return (
        <MapDrawShapeButton
            drawMode="marker"
...
const MapDrawPolygon = ({
    ...
    return (
        ...
        >
            <FontAwesomeIcon icon={faPentagon} />
        </MapDrawShapeButton>
    )
}

const MapDrawActionButton = <T extends EditToolbar.Edit | EditToolbar.Delete>({
    drawAction,
    createDrawTool,
    controlRef,
    className,
...
const MapDrawUndo = ({ className, ...props }: ComponentProps<'button'>) => {
    const drawContext = useMapDrawContext()
...
const useMapDrawHandleIcon = () => {
    ...
    return L.divIcon({
        ...
        html: renderToString(
            <FontAwesomeIcon
                icon={farCircle}
                className="text-primary size-4 transition-transform hover:scale-110"
            />
        )
    })
}
...
const useDebounceLoadingState = (delay = 200) => {
